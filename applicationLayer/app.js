...
// If a new user is registering, his/her data is stored in the userdata.json file
app.post("/registerNewUser",function(req,res){	
	fs.readFile('./datalayer/userdata.json', 'utf8', function(err, data){
		if (err) throw err;
		var user_data =JSON.parse(data);
		user_data.push({first_name: req.body.firstName, last_name:req.body.lastName, email_id: req.body.email, street_name: req.body.street, city_add: req.body.city, postal_code: req.body.postalCode, province_name: req.body.province, password: req.body.password });
...
// Checks if the username exists and if the password matches the username if yes, a new customer is created.
app.post("/signin",function(req,res){
	fs.readFile(__dirname+'/datalayer/userdata.json', 'utf8', function(err, data){
		if (err)  
			throw err;
		var user_data =JSON.parse(data);
		var chck=-1;
		...
		if(user_data[chck].password==req.body.password){
			var currentUser = new Customer(user_data[chck].first_name,user_data[chck].last_name,user_data[chck].email_id,user_data[chck].street_name,user_data[chck].city_add,user_data[chck].postal_code,user_data[chck].province_name);
